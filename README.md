...
1. malloc, 可用空间是分散的一块块的，用 ***空闲链表*** 标识。
    ...
    * malloc分配一块连续的内存。寻找合适的block，first fit；best fit
2. free，把内存块标记未被使用。
---
构造函数 析构函数 虚函数
...
3. 纯虚析构函数
想把Base做出抽象类，不能直接构造对象；需要在其中定义一个纯虚函数。如果其中没有其他合适的函数，可以把析构函数定义为纯虚。
...
---
### new 和malloc区别
1. malloc是库函数, malloc返回的是void*指针
2. new是运算符   ，new建立对象，自动调用构造函数，析构函数。
...
---
### 智能指针
1. shared_ptr，可以有多个指针同时指向同一块内存，底层用引用计数的方式实现，即每增加一个指针，该内存的引用计数+1，减少一个指针，内存引用计数-1，当为0的时候，内存就会被清除。
2. unique_ptr，对象里存放的内容在整个程序中只能出现一次
3. weak_ptr，引用计数会带来循环引用产生无法删除的情况，导致内存泄漏。可使用weak_ptr，弱引用只引用但不增加或删减计数。如果一块内存被shared_ptr和weak_ptr同时引用，当所有的shared_ptr析构了之后，不管有没有weak_ptr引用该内存，内存会被释放。 
---
### extern作用
